...
local function getFileTypeIconWidth()
  local plenaryStrings = require("plenary.strings")
  local devIcons = require("nvim-web-devicons")
  return plenaryStrings.strdisplaywidth(devIcons.get_icon("fname", { default = true }))
end
...
M.buffers = function(options)
  M.prettyBuffersPicker({ picker = "find_files", options = options })
end

M.find_files = function(options)
  M.prettyFilesPicker({ picker = "find_files", options = options })
end

M.oldfiles = function(options)
  M.prettyFilesPicker({ picker = "oldfiles", options = options })
end

M.git_files = function(options)
  M.prettyFilesPicker({ picker = "git_files", options = options })
end

M.live_grep = function(options)
  M.prettyGrepPicker({ picker = "live_grep", options = options })
end

M.grep_string = function(options)
  M.prettyGrepPicker({ picker = "grep_string", options = options })
end
